...
class ControllerProductProduct extends Controller {
	...
	public function index() { 
		...
		//if cached show cached info
		/*$Item = $this->cache->get('item'.$product_id);
		if(!$Item){*/
			$Item = $CallTaobao->QueryItem($data, 0);
			/*$this->cache->set('item'.$product_id, $Item);
		}*/
		...
		if ($Item) {
			...
			$this->data['product_info'] = $product_info = $Item;
...
//			$this->data['manufacturers'] = $this->url->link('product/manufacturer/product', 'manufacturer_id=' . $product_info['manufacturer_id']);
			...
			$this->data['reward'] = 0;
			$this->data['points'] = 0;
			$this->data['stock']  = $product_info['num'];
			
			
			if ($product_info['pic_url']) {
				$this->data['popup'] = $product_info['pic_url'];
			} else {
				$this->data['popup'] = '';
			}
			...
			if ($product_info['pic_url']) {
				...
				$img_thumb = '_'.$this->config->get('config_image_thumb_width').'x'.$this->config->get('config_image_thumb_height').'.jpg';
				$this->data['thumb'] = $product_info['pic_url'].$img_thumb;
			} else {
				$this->data['thumb'] = '';
			}
			...
			if (isset($product_info['item_imgs']['item_img']['url']))
			{
				$result = $product_info['item_imgs']['item_img'];
				$this->data['images'][] = array(
					'addit' => $result['url'].'_310x310.jpg',
					'popup' => $result['url'],
					'thumb' => $result['url'].'_40x40.jpg'
				);
			}
			else
			{
				foreach ($product_info['item_imgs']['item_img'] as $result) {
			...
			if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
				$this->data['price'] = $this->currency->format($this->tax->calculate($product_info['price'], null, $this->config->get('config_tax')));
			} else {
			...
			*/		
			...
			$results = array();//$this->model_catalog_product->getProductRelated($this->request->get['product_id']);
			...
			$results = array();//$this->model_catalog_product->getProductTags($this->request->get['product_id']);
	...
	function loadSkuPrice()
	{
		...
		if (file_exists($file))
		{
			...
			{
				$price = $this->currency->format($this->tax->calculate($aSku[$sku]['price'], null, $this->config->get('config_tax')));
				$sku_price = array('price' => $price);
				//echo $aSku[$sku]['price'];
			}
			else
			{
				$sku_price = array('price' => '');
			}
		}
		else
		{
