		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "return_history SET return_id = '" . (int)$return_id . "', return_status_id = '" . (int)$data['return_status_id'] . "', notify = '" . (isset($data['notify']) ? (int)$data['notify'] : 0) . "', comment = '" . $this->db->escape(strip_tags($data['comment'])) . "', date_added = NOW()");
      	...
      	if ($data['notify']) {
			...
			if ($return_query->num_rows) {
				...
				$language_ru->load('mail/return');
				
				$subject = sprintf($language_ru->get('text_subject'), $this->config->get('config_name'), $return_id);

				$message  = $language_ru->get('text_return_id') . ' ' . $return_id . "\n";
				$message .= $language_ru->get('text_date_added') . ' ' . date($language_ru->get('date_format_short'), strtotime($return_query->row['date_added'])) . "\n\n";
				$message .= $language_ru->get('text_return_status') . "\n";
				$message .= $return_query->row['status'] . "\n\n";

				if ($data['comment']) {
					$message .= $language_ru->get('text_comment') . "\n\n";
					$message .= strip_tags(html_entity_decode($data['comment'], ENT_QUOTES, 'UTF-8')) . "\n\n";
				}

				$message .= $language_ru->get('text_footer');
