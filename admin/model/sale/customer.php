...
class ModelSaleCustomer extends Model {
	...
	public function approve($customer_id) {
		...
		if ($customer_info) {
			...
			$language_ru->load('mail/customer');
			...
			$message  = sprintf($language_ru->get('text_approve_welcome'), $store_name) . "\n\n";
			$message .= $language_ru->get('text_approve_login') . "\n";
			$message .= $store_url . "\n\n";
			$message .= $language_ru->get('text_approve_services') . "\n\n";
			$message .= $language_ru->get('text_approve_thanks') . "\n";
			$message .= $store_name;
			...
			$mail->setSubject(sprintf($language_ru->get('text_approve_subject'), $store_name));
	...
	public function addTransaction($customer_id, $description = '', $amount = '', $order_id = 0) {
		...
		if ($customer_info) { 
			...
			$message  = sprintf($language_ru->get('text_transaction_received'), $this->currency->format($amount, $this->config->get('config_currency'), 1)) . "\n\n";
			$message .= "Описание: " . sprintf($description) . "\n\n";
			$message .= sprintf($language_ru->get('text_transaction_total'), $this->currency->format($this->getTransactionTotal($customer_id), null, 1));
			...
			$mail->setFrom($this->config->get('config_email'));
			$mail->setSender($store_name);
			$mail->setSubject(sprintf($language_ru->get('text_transaction_subject'), $this->config->get('config_name')));
	...
	public function addReward($customer_id, $description = '', $points = '', $order_id = 0) {
		...
		if ($customer_info) { 
			...
			$message  = sprintf($language_ru->get('text_reward_received'), $points) . "\n\n";
			$message .= sprintf($language_ru->get('text_reward_total'), $this->getRewardTotal($customer_id));
			...
			$mail->setFrom($this->config->get('config_email'));
			$mail->setSender($store_name);
			$mail->setSubject(sprintf($language_ru->get('text_reward_subject'), $store_name));
			$mail->setText($message);
			$mail->send();
		}
	}
