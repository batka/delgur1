...
class ModelSaleAffiliate extends Model {
	...
	public function approve($affiliate_id) {
		...
		if ($affiliate_info) {
			...
			$language_ru->load('mail/affiliate');
	
			$message  = sprintf($language_ru->get('text_approve_welcome'), $this->config->get('config_name')) . "\n\n";
			...
			$mail->setSubject(sprintf($language_ru->get('text_approve_subject'), $this->config->get('config_name')));
	...
	public function addTransaction($affiliate_id, $description = '', $amount = '', $order_id = 0) {
		...
		if ($affiliate_info) { 
			...
			$message .= sprintf($language_ru->get('text_transaction_total'), $this->currency->format($this->getTransactionTotal($affiliate_id), $this->config->get('config_currency'), 1));
